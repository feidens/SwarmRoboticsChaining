...
function step_b_chain()
	...
	if (valid_chain_member == 1) then
		...
		--log("r_chain" .. r_chain)
		--log("p_chain" .. p_chain)
		if (r_chain <= p_chain) and (leave_chain_stigma > 400) then
			wait_chain_time = 50
			behavior_change(b_exploration)
		end
...
function calcPChain()
	...
	p_chain = robot.random.uniform() -  0.3*( math.pow( ( 1 + count_chain_sum ), 2 ) /  ( math.pow( ( 1 +  count_messages_sum), 2 ) ) ) - 20 * (1 - groundIsWhite)
	p_chain = p_chain / 2
...
function behavior_change(new_behavior)
	wait_time = robot.random.uniform() * 20 + 25
...
function step_b_nest()
	...
	--log("r_chain" .. r_chain)
	--log("p_chain" .. p_chain)
	if (20 > p_nest) or (r_nest < 0.02) then
		reset_all()
		setSpeed(x_Velo, 0)
		behavior_change(b_exploration)
	end
