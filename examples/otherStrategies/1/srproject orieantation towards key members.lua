...
function step()
	...
	if (part_of_chain == 0) and (collision == 0)then
		...
		-- search the next chain member
		if (search_prev_chain_member ~= 0) then

			setSpeed(-x_Velo, 0)
			search_prev_chain_member = search_prev_chain_member - 1
			if(count_message_recieved_in(3) >= 1) then

				search_prev_chain_member = 0
				setSpeed(0,0)
		...
		for key,tab in pairs(t) do
			...
			if(type(tab) == "table") then
				...
				--	log ("Range : " .. tab.range)
					...
					local lennardJones =  1 --lennard_jones(range)
					...
					if (temp_angle <= math.pi - math.pi/6) and (temp_angle >= math.pi/6) then
						x = x + temp_x
						y = y + temp_y
...
if(collision == 0) then
	...
	if (maxValue > 0.02) then 
		if (closeValueLeft >= closeValueRight) then -- left
			...
			collisionRight = 0
			collision = 1
			robot.wheels.set_velocity(x_Velo,-y_Velo/2)
			--robot.leds.set_all_colors("yellow")
			...
			collisionLeft = 0
			collision = 1
			robot.wheels.set_velocity(-x_Velo/2,y_Velo)
			--robot.leds.set_all_colors("red")
		end
