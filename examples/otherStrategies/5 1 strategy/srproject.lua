...
function step()
	...
	oldGroundIsWhite = groundIsWhite
	...
	if (oldGroundIsWhite - groundIsWhite <= 0) and (groundIsWhite ~= 0) then


		--log("old" .. oldGroundIsWhite)
		--log("new" .. groundIsWhite)
	end
	...
	if ((robot.motor_ground[1].value + robot.motor_ground[2].value) / 2 == 1) and ((robot.motor_ground[3].value + robot.motor_ground[4].value) / 2 < 1) then
		...
		-- become nest keeper
		--log("Leaving nest")
		if(count_message_recieved_in(3) == 0) then
	...
	if (part_of_chain == 0) and (collision == 0)then
		...
		if (leaves_nest == 1) and (part_of_chain == 1) then -- robot leaves the nest and tries to become nest keeper
	...
	if (groundIsWhite == 0)then
		...
		if (onPrey == 0) then
			robot.range_and_bearing.set_data(3, 2)
			found_prey = 1
			x_Velo = 0
			y_Velo = 0
	...
	else -- gound is no longer black so move on
		if(found_prey == 1) then
			x_Velo = 30
			y_Velo = 30
		end
	end
